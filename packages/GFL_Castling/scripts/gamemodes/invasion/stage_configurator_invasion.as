...
class StageConfiguratorInvasion : StageConfigurator {
	...
	protected void setupGlobalConfigs() {
		...
		m_playerAiCompensation_offset = 0;
	}
	...
	protected Stage@ setupStage110() {
		Stage@ stage = createStage();
		stage.m_mapInfo.m_name = "Rising Sand";
		stage.m_mapInfo.m_path = "media/packages/GFL_Castling/maps/map110";
		stage.m_mapInfo.m_id = "map110";

		stage.addTracker(PeacefulLastBase(m_metagame, 0));
		stage.addTracker(CommsCapacityHandler(m_metagame));
		stage.m_maxSoldiers = 220;                                             // was 12*7 in 1.65, 1 base added
		...
		{
			Faction f(getFactionConfigs()[FactionIndex[0]], createCommanderAiCommand(1,0.7,0.2));
			f.m_overCapacity = 30;                                               
			f.m_capacityOffset = 15;
			f.m_capacityMultiplier = 1.0;                                                 
			stage.m_factions.insertLast(f);                                         
		...
		{
			Faction f(getFactionConfigs()[FactionIndex[1]], createCommanderAiCommand(2,0.7,0.2));
			f.m_capacityOffset = 30;
			f.m_capacityOffset = 15;
			f.m_capacityMultiplier = 1.0;                                                
			stage.m_factions.insertLast(f);                                         
	...
	protected Stage@ setupStage1_rust() {
		...
		stage.m_mapInfo.m_id = "map2_c";
		
		stage.addTracker(PeacefulLastBase(m_metagame, 0));
		stage.addTracker(CommsCapacityHandler(m_metagame));
		stage.m_maxSoldiers = 12 * 13; 
...
//		stage.addTracker(Spawner(m_metagame, 1, Vector3(658,10,374), 10));        // vulcan slot filler
	...
	protected Stage@ setupStage14() {
		Stage@ stage = createStage();
		stage.m_mapInfo.m_name = "Rattlesnake Crescent";
		stage.m_mapInfo.m_path = "media/packages/GFL_Castling/maps/map6";
...
// attack score is taken from the map1_2 attack handler script, if bases => 11    
	...
	protected Stage@ setupStage21() {
		...
		stage.m_mapInfo.m_path = "media/packages/GFL_Castling/maps/map20";
	...
	protected Stage@ setupStage22() {
		Stage@ stage = createStage();
		stage.m_mapInfo.m_name = "Hell's High Way";
		stage.m_mapInfo.m_path = "media/packages/GFL_Castling/maps/map21";
	...
	protected Stage@ setupFinalStage2() {
		...
		stage.m_mapInfo.m_name = "Operation Homecoming";
		...
		stage.m_maxSoldiers = 140;                                               // was 80 in 1.70
		stage.m_playerAiCompensation = 2 + m_playerAiCompensation_offset;                                       // was 5 (test4)
		stage.m_playerAiReduction = 0;                                              // wasn't set in 1.65, thus 0
		...
		}
		{
			...
			command.setStringAttribute("class", "faction_resources");
			command.setIntAttribute("faction_id", 1);
			addFactionResourceElements(command, "vehicle", array<string> = {"radio_jammer3.vehicle", "sf_jupiter.vehicle"}, true);
			stage.m_extraCommands.insertLast(command);
		}
